...
def test_bias_visualizations():
    ...
    try:
                ...
                if "component_metrics" in metrics["pair_1"]["metrics"]:
                    ...
                    for comp_name, comp_data in comp_metrics.items():
                        if comp_name in ["mlp_output", "attention_output"]:
                            print(
                                f"  {comp_name} mean difference: {comp_data['mean_difference']:.6f}"
                            )
        except Exception as e:
        ...
        print(
            "Please make sure you have access to the specified model and that your environment is set up correctly."
        )
