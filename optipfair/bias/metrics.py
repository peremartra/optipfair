...
def calculate_activation_differences(
    act1: Dict[str, torch.Tensor], act2: Dict[str, torch.Tensor]
    ...
    for key in act1.keys():
        if key in act2:
            ...
            if act1[key].shape == act2[key].shape:
                ...
                logger.warning(
                    f"Shape mismatch for {key}: {act1[key].shape} vs {act2[key].shape}"
                )
...
def calculate_bias_metrics(
    ...
    for component_type, layer_keys in component_groups.items():
        ...
        mean_diffs = [
            metrics["layer_metrics"][k]["mean_difference"] for k in sorted_keys
        ]
        ...
        progression_metrics = {
            "first_to_last_ratio": last_layer / first_layer
            if first_layer != 0
            else float("inf"),
            "max_to_first_ratio": max_layer / first_layer
            if first_layer != 0
            else float("inf"),
        ...
        if "progression_metrics" not in metrics["component_metrics"][component_type]:
            metrics["component_metrics"][component_type]["progression_metrics"] = (
                progression_metrics
            )
