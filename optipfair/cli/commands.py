...
# @cli.command()
# @click.option(
#     "--model-path", required=True, help="Path or identifier of the model to prune."
# )
# @click.option(
#     "--pruning-type",
#     default="MLP_GLU",
#     type=click.Choice(["MLP_GLU", "DEPTH"]),
#     help="Type of pruning to apply.",
# )
# @click.option(
#     "--method",
#     default="MAW",
#     type=click.Choice(["MAW", "VOW", "PON"]),
...
#     default="last",
#     type=click.Choice(["last", "first", "custom"]),
...
#     "--device",
#     default="auto",
#     help="Device to use for computation (auto, cpu, cuda, cuda:0, etc.)",
# )
# @click.option(
#     "--dtype",
#     default="auto",
#     type=click.Choice(["auto", "float32", "float16", "bfloat16"]),
...
#                 "--pruning-percentage and --expansion-rate are mutually exclusive."
#             )

#         if pruning_percentage is None and expansion_rate is None:
#             pruning_percentage = 10
#             logger.info(
#                 f"No pruning target specified, defaulting to {pruning_percentage}% pruning."
#             )

#         # Validate that depth-specific parameters are not used
#         if num_layers_to_remove is not None:
#             raise click.UsageError(
#                 "--num-layers-to-remove is only valid with --pruning-type DEPTH."
#             )
#         if layer_indices is not None:
#             raise click.UsageError(
#                 "--layer-indices is only valid with --pruning-type DEPTH."
#             )
#         if layer_selection_method != "last" and (
#             pruning_percentage is not None or expansion_rate is not None
#         ):
#             raise click.UsageError(
#                 "--layer-selection-method is only valid with --pruning-type DEPTH."
#             )
...
#         depth_params = [
#             p
#             for p in [num_layers_to_remove, layer_indices, pruning_percentage]
#             if p is not None
#         ]

#         if len(depth_params) == 0:
#             raise click.UsageError(
#                 "For DEPTH pruning, specify one of: --num-layers-to-remove, --layer-indices, or --pruning-percentage."
#             )

#         if len(depth_params) > 1:
#             raise click.UsageError(
#                 "For DEPTH pruning, --num-layers-to-remove, --layer-indices, and --pruning-percentage are mutually exclusive."
#             )

#         # Validate that MLP-specific parameters are not used
#         if expansion_rate is not None:
#             raise click.UsageError(
#                 "--expansion-rate is only valid with --pruning-type MLP_GLU."
#             )
#         if method != "MAW":
#             raise click.UsageError(
#                 "--method is only valid with --pruning-type MLP_GLU."
#             )
...
#                     int(idx.strip()) for idx in layer_indices.split(",")
#                 ]
#             except ValueError:
#                 raise click.UsageError(
#                     "--layer-indices must be comma-separated integers (e.g., '2,5,8')."
#                 )
...
#     if pruning_type == "MLP_GLU":
#         logger.info(
#             f"Pruning model with {pruning_type} pruning, {method} neuron selection method"
#         )
...
#     elif pruning_type == "DEPTH":
#         logger.info(
#             f"Pruning model with {pruning_type} pruning, {layer_selection_method} layer selection method"
#         )
#         if num_layers_to_remove is not None:
...
#     logger.info(
#         f"Reduction: {stats['reduction']:,} parameters ({stats['percentage_reduction']:.2f}%)"
#     )
...
# @cli.command()
# @click.option(
#     "--model-path", required=True, help="Path or identifier of the model to analyze."
# )
# @click.option(
#     "--device",
#     default="auto",
...
#         logger.info(
#             f"Expansion ratio: {expansion_ratio:.2f}x ({expansion_ratio * 100:.2f}%)"
#         )
...
#         )
#         mlp_params = sum(
#             p.numel() for name, p in first_layer.named_parameters() if "mlp" in name
#         )
#         other_params = sum(
#             p.numel()
#             for name, p in first_layer.named_parameters()
...
#             param_reduction = (intermediate_size - new_intermediate_size) * (
#                 hidden_size + mlp.down_proj.out_features
#             )
#             model_reduction_percent = (
#                 (param_reduction * len(layers)) / total_params * 100
#             )

#             logger.info(f"\nWith {prune_percent}% MLP neuron pruning:")
#             logger.info(
#                 f"  New expansion ratio: {new_expansion_ratio:.2f}x ({new_expansion_ratio * 100:.2f}%)"
#             )
#             logger.info(
#                 f"  Estimated parameter reduction: {model_reduction_percent:.2f}% of model"
#             )
