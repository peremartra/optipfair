...
def _calculate_cosine_importance(
    input_tensor, output_tensor, layer_idx, is_first_batch=False
):
    """
...
def analyze_layer_importance(model, dataloader, layers_path=None, show_progress=True):
    ...
    hooks, layer_inputs, layer_outputs, num_layers = _setup_layer_hooks(
        model, layers_path
    )
...
def prune_model_depth(
    ...
    logger.info(
        f"Starting depth pruning: removing {num_layers_to_remove} layers from {total_layers} total layers"
    )
    ...
    logger.info(
        f"Removing layers: {layers_to_remove} using method '{layer_selection_method}'"
    )
