...
class DepthPruner(BasePruner):
    """
    DepthPruner - Class for removing entire transformer layers from models.

    This class provides functionality to prune complete transformer layers,
    ...
    def _analyze_layer_importance(
        ...
        if layers_path is None:
            layers_path = self._infer_layers_path(model)
            if layers_path is None:
                raise ValueError(
            ...
            with torch.no_grad():
                for batch_idx, batch in enumerate(iterator):
                    ...
                    for layer_idx in range(num_layers):
                        ...
                        importance = self._calculate_cosine_importance(
                            input_tensor,
                            output_tensor,
                            layer_idx,
        ...
        final_scores = self._aggregate_importance_scores(layer_importance_scores)
    ...
    def _aggregate_importance_scores(self, layer_scores):
        """
    ...
    def _calculate_cosine_importance(
        self, input_tensor, output_tensor, layer_idx, is_first_batch=False
    ):
        """
    ...
    def _find_layers_by_inspection(self, model):
        """
            ...
            try:
                for attr_name in dir(obj):
                    ...
                    try:
                        ...
                        new_path = (
                            f"{current_path}.{attr_name}" if current_path else attr_name
                        )

                        # Check if this attribute is a valid layer container
                        if self._is_valid_layer_container(attr_value):
                            return new_path
    ...
    def _is_valid_layer_container(self, container):
        """
        ...
        for pattern in LAYER_PATH_PATTERNS:
            try:
                ...
                for part in parts:
                    ...
                    if self._is_valid_layer_container(current):
                        return pattern
    ...
    def _remove_layers_from_model(
        ...
        layer_iterator = (
            tqdm(enumerate(layers), total=len(layers), desc="Removing layers")
            if show_progress
            else enumerate(layers)
        )
        ...
        logger.info(
            f"Removed {len(layer_indices_to_remove)} layers. Model now has {len(new_layers)} layers."
        )

        return model

    def _select_layers_to_remove(
        self,
        total_layers: int,
        ...
        if layer_selection_method == "last":
            ...
            if custom_indices is None:
                raise ValueError(
                    "custom_indices must be provided when layer_selection_method is 'custom'"
                )
    ...
    def _validate_layer_removal_params(
        ...
        param_count = sum(
            1
            for p in [num_layers_to_remove, layer_indices, depth_pruning_percentage]
            if p is not None
        )

        if param_count == 0:
            raise ValueError(
                "Must specify one of: num_layers_to_remove, layer_indices, or depth_pruning_percentage"
            )

        if param_count > 1:
            raise ValueError(
                "Parameters num_layers_to_remove, layer_indices, and depth_pruning_percentage are mutually exclusive"
            )
        ...
        if layer_selection_method not in valid_methods:
            raise ValueError(
                f"layer_selection_method must be one of {valid_methods}, got {layer_selection_method}"
            )
        ...
        if num_layers_to_remove is not None:
            ...
            if num_layers_to_remove >= total_layers:
                raise ValueError(
                    f"Cannot remove {num_layers_to_remove} layers from model with {total_layers} layers"
                )
        ...
        if depth_pruning_percentage is not None:
            ...
            if num_layers_to_remove >= total_layers:
                raise ValueError(
                    f"depth_pruning_percentage {depth_pruning_percentage}% would remove all layers"
                )
            if num_layers_to_remove == 0:
                raise ValueError(
                    f"depth_pruning_percentage {depth_pruning_percentage}% would remove 0 layers"
                )
        ...
        if layer_indices is not None:
            ...
            if not all(0 <= idx < total_layers for idx in layer_indices):
                raise ValueError(
                    f"All layer_indices must be between 0 and {total_layers - 1}"
                )
            if len(set(layer_indices)) != len(layer_indices):
                raise ValueError("layer_indices must not contain duplicates")
            if len(layer_indices) >= total_layers:
                raise ValueError(
                    f"Cannot remove {len(layer_indices)} layers from model with {total_layers} layers"
                )
        ...
        return ValidateLayerRemovalParamsReturn(
    ...
    def prune(
        ...
        **kwargs,
        ...
        config = self._validate_layer_removal_params(
            ...
            layer_selection_method=parsed_kwargs.layer_selection_method,
        )
        ...
        if config.layer_selection_method == "custom":
            layers_to_remove = config.layer_indices
        else:
            layers_to_remove = self._select_layers_to_remove(
        ...
        logger.info(
            f"Removing layers: {layers_to_remove} using method '{config.layer_selection_method}'"
        )
