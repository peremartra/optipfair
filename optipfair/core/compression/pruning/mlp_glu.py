...
_accumulated_act_norms: Dict[int, Tensor] = {}
...
def prune_model_mlp_glu(
    ...
    if not validate_model_for_glu_pruning(model):
        raise ValueError(
            "Model is not compatible with GLU pruning. It must have gate_proj, up_proj, and down_proj layers."
        )
    ...
    if neuron_selection_method not in IMPORTANCE_FUNCTIONS:
        raise ValueError(
            f"Invalid neuron selection method: {neuron_selection_method}. "
    ...
    if pruning_percentage is not None and expansion_rate is not None:
        raise ValueError(
            "pruning_percentage and expansion_rate are mutually exclusive. Provide only one."
        )
    ...
    if expansion_rate is not None:
        ...
        logger.info(
            f"Calculated pruning percentage: {pruning_percentage:.2f}% to achieve "
            f"expansion rate of {expansion_rate}%"
        )

    # Ensure pruning_percentage is available and within valid range
    if pruning_percentage is None:
        # At this point expansion_rate was not provided (would have converted it)
        raise ValueError("Either pruning_percentage or expansion_rate must be provided")

    # Narrow type for mypy and ensure it's a float
    pruning_percentage = float(pruning_percentage)

    if not 0 <= pruning_percentage <= 100:
        raise ValueError(
            f"pruning_percentage must be between 0 and 100, got {pruning_percentage}"
        )
