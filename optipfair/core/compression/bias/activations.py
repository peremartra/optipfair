...
def register_hooks(model) -> List[Any]:
    ...
    try:
        ...
        # BERT-style models with model.encoder.layer
            ...
            logger.warning(
                "Model architecture not fully recognized. Attempting to locate layers..."
            )
...
def get_activation_pairs(
    model: Any, tokenizer: Any, prompt1: str, prompt2: str
) -> tuple:
    """
...
def get_layer_names(
    activations: Dict[str, torch.Tensor], layer_type: Optional[str] = None
) -> List[str]:
    """
...
def select_layers(
    layer_names: List[str], selection: Union[str, List[int]] = "first_middle_last"
) -> List[str]:
    """
