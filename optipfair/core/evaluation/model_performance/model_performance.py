...
from core.evaluation.model_performance.types.compute_perplexity_for_batch_return import ComputePerplexityForBatchReturn
...
class ModelPerformanceBenchmarker:
    ...
    def _compute_perplexity_for_batch(
        ...
        negative_log_likelihood: torch.Tensor = -target_log_probs.sum(dim=-1) / attention_mask[:, 1:].sum(dim=-1)
    ...
    def evaluate_perplexity(
        ...
        mean_perplexity: float = sum(all_perplexities) / len(all_perplexities) if all_perplexities else 0.0
