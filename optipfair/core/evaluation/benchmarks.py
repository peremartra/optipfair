...
class ModelBenchmarker:
    ...
    def compare_models_inference(self, original_benchmark: InferencePerformanceInfo, compressed_benchmark: InferencePerformanceInfo) -> CompareBenchmark:
        speedup = (
            original_benchmark / compressed_benchmark.avg_time if compressed_benchmark.avg_time > 0 else float("inf")
        )
        tps_improvement = (
            (compressed_benchmark.tokens_per_second / original_benchmark.tokens_per_second - 1) * 100
