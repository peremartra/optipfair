...
for pruning_pct in PRUNING_PERCENTAGES:
    ...
    print("   ✅ Static pruning complete")
    print(f"      Pruned parameters: {static_params:,}")
    print(
        f"      Reduction: {stats_static['reduction']:,} ({stats_static['percentage_reduction']:.2f}%)"
    )
    print(f"      New intermediate size: {static_intermediate}")
    static_expansion_rate = (
        float(stats_static["expansion_rate"])
        if stats_static.get("expansion_rate") is not None
        else 0.0
    )
    print(f"      Expansion rate: {static_expansion_rate:.2f}%")
    ...
    print(f"   Original parameters: {original_params:,}")
    print(f"   Original intermediate size: {original_intermediate}")
    print("   Running calibration + pruning...")
    ...
    print(
        f"      Reduction: {stats_hybrid['reduction']:,} ({stats_hybrid['percentage_reduction']:.2f}%)"
    )
    print(f"      New intermediate size: {hybrid_intermediate}")
    hybrid_expansion_rate = (
        float(stats_hybrid["expansion_rate"])
        if stats_hybrid.get("expansion_rate") is not None
        else 0.0
    )
    print(f"      Expansion rate: {hybrid_expansion_rate:.2f}%")
    ...
    print(
        f"   {'Total Parameters':<30} {static_params:<20,} {hybrid_params:<20,} {'✅ YES' if params_match else '❌ NO':<10}"
    )
    print(
        f"   {'Intermediate Size':<30} {static_intermediate:<20} {hybrid_intermediate:<20} {'✅ YES' if intermediate_match else '❌ NO':<10}"
    )
    print(
        f"   {'Expansion Rate':<30} {static_expansion_rate:<20.2f} {hybrid_expansion_rate:<20.2f} {'✅ YES' if abs(static_expansion_rate - hybrid_expansion_rate) < 0.01 else '❌ NO':<10}"
    )
...
for pruning_pct, result in results.items():
    ...
    if result["sizes_match"]:
        print("   Both methods produced identical model sizes:")
        print(f"   - Parameters: {result['static']['parameters']:,}")
        print(f"   - Intermediate size: {result['static']['intermediate_size']}")
        exp_rate_static = (
            float(result["static"]["expansion_rate"])
            if result["static"].get("expansion_rate") is not None
            else 0.0
        )
        print(f"   - Expansion rate: {exp_rate_static:.2f}%")
    else:
        print("   ⚠️  Size mismatch detected:")
        print(
            f"   Static:  {result['static']['parameters']:,} params, intermediate={result['static']['intermediate_size']}"
        )
        print(
            f"   Hybrid:  {result['hybrid']['parameters']:,} params, intermediate={result['hybrid']['intermediate_size']}"
        )
        all_passed = False
